...
public class TagmeParser
{
	...
	public void parse(AnnotatedText input)
	{
		...
		int[] tokens   = new int[copy.length];
		int[] tsize    = {0};
		int[] indexes  = {0, 0, 0, 0};
		input.text_len = TagmeParser.clean_new(input.original, copy, input.text, input.offsets, input.breaking_pos, tokens, tsize, indexes, 0);
		...
		for (int currentToken = -1; currentToken < tokens_len; currentToken++)
		{
			...
			for (int tokenIdx=0; tokenIdx<windowSize; tokenIdx++)
			{
				...
				if (tokenizedWindow.anchors[tokenIdx] != null)
				{
					...
					// System.out.println(minLinkProb > Anchor.MIN_LP && s.anchor.lp() < minLinkProb);
	...
	// Antonio Cleaner
	private static void add_space(int[] indexes, char[] output, int[] offsets, int[] tokens, int real_end_pos){
		tokens[indexes[1]++]  = indexes[2];
		output[indexes[2]++] = ' ';
		offsets[indexes[3]++] = real_end_pos;
	}

	private static void add_elements(char[] input, char[] output, int[] offsets, int[] indexes, int[] tokens, int pos, int offset){
		int real_end_pos = offset + pos;
		int n_elements   = real_end_pos - indexes[0];
		if (n_elements > 0) {
			System.arraycopy(input, indexes[0], output, indexes[2], n_elements);
			IntStream.range(indexes[0], real_end_pos).forEach(i -> offsets[indexes[3]++] = i);
			indexes[2] += n_elements;
			if(real_end_pos != input.length)
				add_space(indexes, output, offsets, tokens, real_end_pos);
		}
		else if(indexes[2] > 0 && output[indexes[2]-1] != ' ') {
			add_space(indexes, output, offsets, tokens, real_end_pos);
		}
	}


	public static LinkedList<Integer[]> text_regex(String text) {
		String  regex_1    = "\\s[\\[({][^\\[({]+[])}][\\s,]";
		String  regex_2    = "[^0-9a-zA-Z(){}\\[\\]+]\\s|\\s[^0-9a-zA-Z(){}\\[\\]+]|\\s";
		Pattern regex_expr = Pattern.compile(regex_1 + "|" + regex_2);
		Matcher matcher    = regex_expr.matcher(text);
		LinkedList<Integer[]> results = new LinkedList<>();
		while (matcher.find()) {
			Integer[] extremes = {matcher.start(), matcher.end()};
			results.add(extremes);
		}
		return results;
	}


	public static int clean_new(
			MutableString origin, char[] input, char[] output, int[] offsets,
			boolean[] breakingChars, int[] tokens, int[] tsize, int[] indexes, int offset
	){
		if (origin.length() == 0) return -1;
		int in_len = input.length;
		for(int j=0; j<in_len; j++)
			input[j] = UTF16toASCII.MAP[input[j]];
		String origin_text = origin.toString();
		LinkedList<Integer[]> results = text_regex(origin_text);
		for(Integer[] record: results){
			add_elements(input, output, offsets, indexes, tokens, record[0], offset);
			if (record[1] - record[0] > 2){
				indexes[0] = offset + record[0] + 2;
				breakingChars[indexes[2]-1] = true;
				MutableString sub_txt = origin.substring(indexes[0], record[1] - 2);
				clean_new(sub_txt, input, output, offsets, breakingChars, tokens, tsize, indexes, indexes[0]);
				breakingChars[indexes[2]-1] = true;
			}
			indexes[0] = offset + record[1];
		}
		int size = input[in_len -1 ] == '.' ? origin.length() - 1 : origin.length();
		add_elements(input, output, offsets, indexes, tokens, size, offset);
		tsize[0] = indexes[1];
		return indexes[2];
	}
	// Fine Antonio Cleaner
